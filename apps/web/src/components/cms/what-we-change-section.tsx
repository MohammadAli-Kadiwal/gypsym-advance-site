...
function SpeedMetricItem({ metric, index }: { metric: { label: string; value: string; percent: number }; index: number }) {
  ...
  const targetWidth = Math.max(10, Math.min(100, metric.percent || 80));
  ...
  return (
    <div ref={barRef} className="space-y-1.5">
      <div className="flex items-center justify-between text-xs font-semibold text-neutral-700 px-0.5">
        ...
        <AnimatedCounter value={metric.value} duration={1.5} delay={index * 150} />
      </div>
      <div className="h-2 w-full bg-white/70 rounded-full overflow-hidden">
        <div
          style={{
            width: inView ? `${targetWidth}%` : '5%',
            transitionDelay: `${index * 120}ms`,
          }}
          className="h-full bg-[#d9127b] rounded-full transition-all duration-700 ease-out"
        />
      </div>
    </div>
  );
  ...
  return (
    <div className="w-full py-8 sm:py-10 md:py-12">
      <div className="max-w-[1360px] mx-auto px-4 sm:px-6 md:px-8">
        ...
        {/* 3-Column Cards Grid */}
        <StaggerContainer preset="normal" className="grid grid-cols-1 md:grid-cols-3 gap-5 sm:gap-8">
          {cards.map((card: any, index: number) => {
            const cardType = card.type || (index === 0 ? 'cart' : index === 1 ? 'speed' : 'theme');

            return (
              <RevealItem key={card.id || index}>
                <div
                  className="bg-white rounded-[22px] sm:rounded-[30px] p-5 sm:p-7 border border-neutral-200/70 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.03)] flex flex-col justify-between hover:shadow-md transition-shadow duration-200 h-full"
                >
                  ...
                  <div className="h-44 sm:h-48 rounded-2xl bg-[#fce7ec] p-5 sm:p-6 flex flex-col justify-center items-stretch mb-6 relative overflow-hidden">
                    ...
                    {cardType === 'cart' && (() => {
                      const checkoutText = card.checkoutButtonText || 'Checkout · $89.00';
                      const parts = checkoutText.split('·');
                      return (
                        <div className="w-full space-y-2.5">
                          <div className="bg-white rounded-xl py-2.5 px-4 flex items-center justify-between shadow-xs">
                            ...
                            <span className="text-[11px] font-bold text-[#d9127b] bg-[#fdf2f4] px-2 py-0.5 rounded-md">
                              <AnimatedCounter value={card.cartStep || '1 step'} duration={1.2} />
                            </span>
                          </div>
                          <div className="w-full py-2.5 px-4 bg-neutral-900 text-white rounded-xl text-xs font-semibold flex items-center justify-center shadow-xs">
                            {parts.length > 1 ? (
                              <>
                                {parts[0].trim()} · <AnimatedCounter value={parts[1].trim()} duration={1.6} delay={100} />
                              </>
                            ) : (
                              <AnimatedCounter value={checkoutText} duration={1.6} />
                            )}
                          </div>
                        </div>
                      );
                    ...
                    {cardType === 'speed' && (
                      <div className="w-full space-y-3.5">
                        {(card.metrics || [
                          ...
                          <SpeedMetricItem key={mIdx} metric={m} index={mIdx} />
                        ))}
                      </div>
                    )}
